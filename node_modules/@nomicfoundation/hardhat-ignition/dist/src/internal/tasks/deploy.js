...
import { ensureDir, exists, readdir, readJsonFile, remove, writeJsonFile, } from "@nomicfoundation/hardhat-utils/fs";
...
const taskDeploy = async ({ modulePath, parameters: parametersInput, deploymentId: givenDeploymentId, defaultSender, reset, verify, strategy: strategyName, writeLocalhostDeployment, }, hre) => {
    const connection = await hre.network.connect();
    const chainId = Number(await connection.provider.request({
        method: "eth_chainId",
    }));
    const deploymentId = resolveDeploymentId(givenDeploymentId === undefined || givenDeploymentId === ""
        ? undefined
        : givenDeploymentId, chainId);
    const deploymentDir = connection.networkConfig.type === "edr-simulated" &&
    ...
    if (chainId !== 31337) {
        ...
        const instanceFilePath = path.join(hre.config.paths.cache, ".hardhat-network-instances.json");
        const instanceFileExists = await exists(instanceFilePath);
        const instanceFile = instanceFileExists ? await readJsonFile(instanceFilePath) : {};
    ...
    if (reset) {
        if (deploymentDir === undefined) {
            throw new HardhatError(HardhatError.ERRORS.IGNITION.INTERNAL.CANNOT_RESET_EPHEMERAL_NETWORK);
        }
        else {
            await remove(deploymentDir);
        }
    ...
    const result = await deploy({
        ...
        strategyConfig,
        maxFeePerGasLimit: hre.config.networks[connection.networkName]?.ignition.maxFeePerGasLimit,
        maxFeePerGas: hre.config.networks[connection.networkName]?.ignition.maxFeePerGas,
        maxPriorityFeePerGas: hre.config.networks[connection.networkName]?.ignition
...
async function resolveParametersString(paramString) {
    try {
        ...
        if (e instanceof Error) {
            throw new HardhatError(HardhatError.ERRORS.IGNITION.INTERNAL.FAILED_TO_PARSE_JSON, e);
        }
        throw e;
    }
}
